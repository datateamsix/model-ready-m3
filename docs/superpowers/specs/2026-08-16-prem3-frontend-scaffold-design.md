...
- ~~Satoshi web assets are not in the repo; Inter is used and documented as the fallback until licensed Satoshi files are added~~ — resolved 2026-08-17: the licensed Satoshi package landed at `brand/brand-assets/fonts/Satoshi_Complete`. Self-hosted via `next/font/local` (`frontend/src/app/layout.tsx`) using only the variable-font `woff2` files (normal + italic, weight 300–900), copied into `frontend/src/fonts/satoshi/`. `--font-display` now resolves to Satoshi with Inter as its own fallback chain; `--font-ui` (body/UI) stays Inter per `prem3.tokens.json`'s `typography.ui` — that split was never in question, only whether Satoshi assets existed to implement `typography.display`.
