...
- `datasets/` (the canonical dataset restructure, Dataset C seal) is **not yet on `origin/main`** — it exists only on the sibling `feature/prem3-dataset-c-summit-pine` branch in the other worktree. This worktree's baseline predates it. Per the mission brief this is expected; rebase happens after that work merges (Section 50 of the brief). No dependency on it for Mission 1.
...
- `datasets/` restructure and Dataset C seal are not yet on `origin/main`; a rebase pass is required later per the mission brief before final handoff, not part of this plan's initial scope.
