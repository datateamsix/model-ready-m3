...
def _authority(state: DurableRunState):
    uri = state.package_uri if str(state.package_uri).startswith("gs://") else "gs://raw/internal/package/"
    return bind_service_execution(
