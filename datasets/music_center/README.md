...
## Naming: two different “Dataset B” packages

`datasets/music_center/dataset_b/` remains this **Music Center** related-schema episode (Meta `amount_spent`, Google Ads / Meta mix).

The independent MEL learning-evidence Dataset B is **Stride & Field** at `datasets/stride_and_field/dataset_b/`, generated by `scripts/generate_dataset_b.py` using shared helpers in `app/synthetic/mmm.py`. Do not overwrite this Music Center Dataset B when generating Stride & Field.
