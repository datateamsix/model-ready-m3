...
import { StartCreateProjectForm } from "./start-create-project-form";

describe("StartCreateProjectForm", () => {
  it("renders a name field and the given submit label", () => {
    render(<StartCreateProjectForm stage="getting-organized" label="Create and start planning" />);
  ...
  it("submits the stage as a hidden field alongside the name", async () => {
    ...
    render(<StartCreateProjectForm stage="ready-to-assess" label="Create and continue" />);
  ...
  it("shows the backend's typed error message instead of a fabricated success", async () => {
    ...
    const user = userEvent.setup();
    render(<StartCreateProjectForm stage="getting-organized" label="Create" />);
